    ...
    if (keyword !== '') {
        ...
        for (let i = 0; i < Monlist.length; i++) {
            ...
            if (Monlist[i].name.toLowerCase().indexOf(keyword) !== -1) {
                ...
                let monsterHtml = '<div class="hove" listId="' + i + '" monId="' + Monlist[i].id + '" onclick="getStdByMon('+i+')">' +
                                 (i+1) + '、' + Monlist[i].name + '</div>';
...
function getStdByMon(monId) {
    ...
    let itemHtml = '';
    const monsterId = Monlist[monId].id;

    // Go through all items to find which ones are dropped by this monster
    for (let i = 0; i < Stdlist.length; i++) {
        // Check if the monster ID is in the item's monster list
        if (Stdlist[i].mon !== '-1') {
            const monsterIds = Stdlist[i].mon.split(',');
            for (let j = 0; j < monsterIds.length; j++) {
                if (String(monsterIds[j]) === String(monsterId)) {
                    itemHtml += '<div class="hove" listId="' + i + '" onclick="getMapByStd('+i+')">' +
...
function getMapList(mapId) {
    ...
    if (typeof maplist !== 'undefined' && maplist[mapId]) {
        ...
        if (maplist[mapId].transfer) {
            $('#mapTransferList').html(mapInfo + '<br>跑图流程:' + maplist[mapId].transfer);
        } else {
